//Checks the number of todolist stored in local storage
...
// list from local storage and render in HTML page with the help of generateItems() function.
function getItems() {
    
  //  loads the 'todoList' object from database .
 
  let db = JSON.parse(localStorage.getItem("todoList")) || [];
 
  if (db.length > 0) {

    //setting active item status to "active" 
    let activeItems = db.filter((item) => item.status === "active");
